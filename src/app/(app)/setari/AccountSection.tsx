  ...
  return (
    <section>
      ...
      <div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
        <form onSubmit={handleSaveName} className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <button
            type="submit"
            disabled={savingName}
            className="rounded-lg bg-white px-4 py-2.5 text-sm font-medium text-black transition hover:bg-neutral-200 disabled:opacity-50"
          >
      ...
      <form
        ...
        className="mt-4 grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-5 sm:grid-cols-2"
      >
        <p className="text-sm font-medium text-white sm:col-span-2">Schimbă parola</p>
        <input
          type="password"
        ...
        <button
          ...
          className="inline-flex w-fit items-center gap-1.5 rounded-lg bg-white/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/20 disabled:opacity-50 sm:col-span-2"
        >
          {passwordSaved && <Check size={14} />}
          {savingPassword ? "Se salvează..." : passwordSaved ? "Parolă schimbată" : "Schimbă parola"}
        </button>
        {passwordError && <p className="text-sm text-red-400 sm:col-span-2">{passwordError}</p>}
      </form>
    </section>
  );
}
